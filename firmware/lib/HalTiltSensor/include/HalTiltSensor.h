...
// QMI8658 gyro flick detection (X3 only; the X4 has no IMU).
// The app is portrait-only: a forward flick maps to "next", a back flick to "previous".
class HalTiltSensor {
 ...
 public:
  // Call after gpio.begin() (I2C already initialised for X3); no-op on X4
  void begin(const HalGPIO& gpio);
  ...
  // Poll the gyro and update tilt gesture state. Wakes/sleeps the sensor to
  // follow `enabled`; while enabled, flicks trigger the tilt events below.
  ...
  // Discard any pending tilt events (call when disabling tilt input).
