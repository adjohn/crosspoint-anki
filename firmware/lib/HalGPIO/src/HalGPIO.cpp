...
HalGPIO::DeviceType detectDeviceTypeWithFingerprint() {
  // Explicit override for recovery/support: 0 = auto, 1 = force X4, 2 = force X3
...
void HalGPIO::begin() {
  // Detect device type first: on X3, GPIO0/GPIO20 are the I2C bus and must
  // not be touched by the X4 battery/USB pin setup.
...
int HalGPIO::getBatteryPercentage() const {
  if (deviceIsX3()) {
    // X3: BQ27220 fuel gauge StateOfCharge()
...
bool HalGPIO::isUsbConnected() const {
  if (deviceIsX3()) {
    // X3: infer USB/charging via BQ27220 Current() register (positive = charging)
    for (uint8_t attempt = 0; attempt < 2; ++attempt) {
      int16_t currentMa = 0;
